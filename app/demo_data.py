...
def load_demo_emails(db: Session):
    existing = set(db.scalars(select(Email.external_id)).all())
    for item in DEMO_EMAILS:
        if item["external_id"] not in existing:
            db.add(Email(**item, source="demo"))
    db.commit()
    return list(db.scalars(select(Email).order_by(Email.received_at.desc())).all())
